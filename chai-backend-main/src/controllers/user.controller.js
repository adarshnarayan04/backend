...
//dont use async handler , async handler is for express reqeust ( req, res, next ) for normal async function
const generateAccessAndRefereshTokens = async (userId) => {
  try {
    const user = await User.findById(userId);
    const accessToken = user.generateAccessToken(); //defined in user.model.js
    const refreshToken = user.generateRefreshToken();
    //it will get the details from user ( can see the codejwt.sign({_id:this._id,emmail..}) )
    ...
    //this error will be handled by the catch of aynsc handler of the login function

    //When an error is re-thrown inside the catch block
    //it will propagate out of that function(mean go to login function , as it is called there). If the function is wrapped by asyncHandler, that re-thrown error ends up being caught by asyncHandler’s catch block (i.e. the .catch((err) => next(err)) part) and passed to Express's error-handling middleware.
  }
};
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    .json(
      //in json we send the ApiResponse constructor(statusCode, data, message = "Success") of ApiResponse.js
...
const logoutUser = asyncHandler(async (req, res) => {
  //to logout we dont send any data by postman
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    {
      $set: {
        //set operator of mongodb is used to update the fields
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    {
      $set: {
        //update the avatar field usign set operator
        avatar: avatar.url,
      },
    },
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    {
      $addFields: {
        //adding the fields which we want to add
        subscribersCount: {
          $size: "$subscribers",
        },
    ...
    {
      $project: {
        //ONLY shoe thr variable name which we want to show
        fullName: 1,
        username: 1,
        subscribersCount: 1,
  ...
  if (!channel?.length) {
    //due to lookup it return array
    throw new ApiError(404, "channel does not exists");
  }

  //the value return goes to fronted as it is respond
  return res.status(200).json(
    //as the aggregrate function return array so we have to access the first element of the array
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    ...
    {
      $lookup: {
        //user(watchhistory)-->video(owner)-->user
        from: "videos",
        ...
        pipeline: [
          //it is subpipeline
          {
            $lookup: {
              ...
              as: "owner",
              pipeline: [
                //subpipeline inside subpipeline
                {
                  $project: {
                    fullName: 1,
                    username: 1,
